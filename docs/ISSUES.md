...
- **Acceptance criteria:** `npm run dev` boots the default page with no errors; all listed dependencies present in `package.json`; `.env.example` committed, `.env.local` gitignored; **`postcss.config.mjs` is present at the repo root and Tailwind utility classes actually appear in the compiled CSS** — a clean `next build`/`next dev` exit code is not sufficient proof of this. Verify by inspecting `.next/static/**/*.css` (or the dev server's served CSS chunk) for a known class like `.flex` or a custom `@theme` token, not just by checking the process exit code.
  - **Bug found post-M0 (worth re-checking on any future scaffold-copy):** `postcss.config.mjs` was missed when copying files from the temp scaffold into the real repo. Without it, `@tailwindcss/postcss` never runs, so Turbopack silently passes `globals.css` through with the `@theme` block unprocessed — it only logs a low-looking "Unknown at rule: @theme" **warning**, the build still exits 0, and pages still statically generate, so nothing in the standard verification steps (lint/build/tsc) catches it. The result: zero Tailwind utility classes are generated at all (not just the custom color tokens) — every layout and color rule silently no-ops. Symptom: the page loads but has no styling — default browser fonts/colors, no flex/grid layout, nothing.
- **Labels:** `setup`
...
- **Description:** Add `.github/workflows/ci.yml` running on `push` to `main` and on every `pull_request`: checkout → `actions/setup-node` (Node **24**, Active LTS as of Aug 2026 — re-verify at build time, this drifts, `cache: npm`) → `npm ci` → `npm run lint` → `npm run build` → `npx tsc --noEmit`. No deploy step in this milestone (deploy pipeline, if wanted, is a separate later decision) — this is verification-only CI, matching SRS NFR-4 (build correctness) so every later PR is checked automatically instead of relying on manual `next build` runs. Cache `~/.npm`/`node_modules` via `setup-node`'s built-in npm cache to keep runs fast.
  ...
  - **Also add a "Verify Tailwind CSS actually compiled" step right after `build`** (discovered post-M0): a missing/misconfigured `postcss.config.mjs` makes Turbopack silently pass `globals.css` through unprocessed — `next build` still exits 0, only a low-visibility "Unknown at rule: @theme" warning is logged, and zero Tailwind utility classes end up in the compiled CSS. Neither `lint` nor `tsc --noEmit` catches this since it's a CSS-output problem, not a JS/TS one. The check: grep the built `.next/static/**/*.css` for a known class (e.g. `.bg-bg`) and fail the job if it's absent.
- **Acceptance criteria:** workflow runs green on a trivial PR against `main`; a deliberately broken build (bad import) or lint error fails the workflow; workflow file references only scripts that exist in `package.json` (`lint`, `build`) plus `tsc --noEmit` for an explicit type-check step, run after `build` so `.next/types` already exists; the CSS-compilation check fails the workflow if `postcss.config.mjs` is missing or misconfigured (verify by temporarily renaming it locally and re-running).
- **Labels:** `setup`, `ci`
